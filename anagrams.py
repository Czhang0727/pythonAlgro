class Solution:
    ...
    #this function if render obj to hashcode
    '''
        {
            'a' : 2,
            'v' : 1
    ...
    def countChar(self, str):
        charMap = [0] * 26
        #since we only have lower case
        baseChar = ord('a')
        for c in str:
            charMap[ord(c) - baseChar] += 1
        
        resObj = {}
        for i in range(len(charMap)):
            if charMap[i] > 0:
                resObj[chr(i + baseChar)] = charMap[i]
        return self.genHash(resObj)        
        
        
    def anagrams(self, strs):
        # write your code here
        # lets do a hashMap to contain all anaMap like : {'a1b1' : ['ab','ba']}
        anaMap = {}
        
        for str in strs:
            hashCode = self.countChar(str)
            if hashCode in anaMap:
                anaMap[hashCode].append(str)
            else:
                # did not find, create an empty array
                anaMap[hashCode] = [str]
